...
d3.json("communes.geojson", function(geoJSON) {
   ...
   d3.csv("pop74.csv", function(csv) {
			var quantile = d3.scale.quantile().domain([0, Math.sqrt(d3.max(csv, function(e) { return +e.POP; }))]).range(d3.range(9));
			legend = svg.append('g')
      	...
      	lg.enter().append('svg:rect')
			...
			csv.forEach(function(e,i) {
    			d3.select("#d74" + e.CODE)
        			.attr("class", function(d) { return "q" + quantile(Math.sqrt(+e.POP)) + "-9"; })
    		});
    });
});
